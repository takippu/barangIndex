...
export const PriceIndexScreen: React.FC<PriceIndexScreenProps> = ({ className = '' }) => {
    ...
    return (
        <div className={`bg-slate-50 font-sans text-slate-900 antialiased min-h-screen pb-24 lg:pb-0 ${className}`}>
            <DesktopHeader activeNav="/markets" />

            <div className="max-w-md mx-auto min-h-screen flex flex-col pb-20 relative bottom-nav-safe lg:max-w-7xl lg:px-8 lg:pt-8 lg:pb-12">
                {/* Mobile Header */}
                <header className="sticky top-0 z-20 bg-slate-50/80 backdrop-blur-xl px-4 py-3 border-b border-slate-200/50 flex items-center justify-between lg:hidden">
                ...
                {/* Desktop Title & Region */}
                <div className="hidden lg:flex items-end justify-between mb-8">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <h1 className="text-3xl font-extrabold text-slate-900">{selectedItem?.name ?? 'Loading...'}</h1>
                            {regionName && (
                                <span className="bg-emerald-50 text-emerald-700 text-xs font-bold px-2 py-1 rounded-full border border-emerald-100 uppercase tracking-wider flex items-center gap-1">
                                    <span className="material-symbols-outlined text-sm">location_on</span>
                                    {regionName}
                                </span>
                            )}
                        </div>
                        <p className="text-slate-500 font-medium">Historical price trends and market statistics</p>
                    </div>
                </div>
                ...
                <div className="lg:grid lg:grid-cols-12 lg:gap-8 lg:items-start">
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        {/* Controls Section */}
                        <section className="px-4 py-6 lg:p-0 space-y-4">
                            <div className="lg:flex lg:items-center lg:justify-between lg:gap-4">
                                ...
                                <div className="flex bg-slate-100 p-1 rounded-xl shrink-0 w-full lg:w-auto">
                                    {(['7d', '30d', '90d', '1y'] as const).map((value) => (
                                        <button
                                            key={value}
                                            className={`flex-1 lg:flex-none lg:px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${timeframe === value ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                        ...
                        {/* Chart Section */}
                        <section className="px-4 pb-6 lg:p-0">
                            <div className="bg-white rounded-2xl border border-slate-100 shadow-soft p-4 lg:p-6 relative overflow-hidden">
                                ...
                                {chart ? (
                                    <div className="relative z-10">
                                        <svg viewBox={`0 0 ${chart.width} ${chart.height}`} className="w-full h-48 lg:h-96 overflow-visible">
                                            <defs>
                                    ...
                                    <div className="py-12 lg:py-32 text-center relative z-10">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <section className="px-4 pb-6 lg:p-0">
                            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-4 px-1 lg:hidden">Market Statistics</h3>
                        ...
                        {/* CTA Section */}
                        <section className="px-4 mb-6 lg:p-0">
